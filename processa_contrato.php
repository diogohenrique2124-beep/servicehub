...
if($cpf && strlen($cpf) != 11){
        header("location: contratar.php?erro=CPF invalido. Digite 11 números.");
    exit();
}
 
if(count($servicos_validos) < 1){
        header("location: contratar.php?erro=Selecione ao menos um srviço válido.");
    exit();
}
...
if($data_preferida){
    ...
    if($ts === false){
        header("location: contratar.php?erro= A Data Invalida.");
        exit();
    

        if($ts < strtrotime(date('Y-m-d'))){
        header("location: contratar.php?erro= A data não pode ser aterior a hoje.");
        exit();
    }
}
    ...
    if (!$usuarioBanco->buscarPorEmail($email)==false){
        // se retornou falso e´por que não tem usuário com este email no banco
        // então gravamos
        $usuario = new Usuario ();
